...
export function NotificationManager({ userId, onBack }: NotificationManagerProps) {
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        const [chRes, tplRes] = await Promise.all([fetchChannels(), fetchTemplates()]);
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-slate-800">
            ...
            <header className="bg-slate-900 text-white shadow-md sticky top-0 z-50">
                <div className="container mx-auto px-4 sm:px-6 py-4 flex justify-between items-center gap-4">
                    ...
                    <button onClick={onBack}
                        className="flex items-center gap-1.5 text-sm text-slate-300 hover:text-blue-400 transition px-2 py-1.5 shrink-0">
            ...
            <main className="flex-1 container mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-8 max-w-3xl">
                <div>
                ...
                <section className="space-y-3">
                    <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">渠道設定</h3>
                    {loading ? (
